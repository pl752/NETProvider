...
internal class GdsStatement : StatementBase
{
	...
	{
		var descriptors = ParseSqlInfo(response.Data, DescribeInfoAndBindInfoItems, [null, null]);
	...
	{
		var descriptors = await ParseSqlInfoAsync(response.Data, DescribeInfoAndBindInfoItems, [null, null], cancellationToken).ConfigureAwait(false);
		_fields = descriptors[0];
		_parameters = descriptors[1];
	}
	#endregion
			...
			while ((item = info[currentPosition++]) != IscCodes.isc_info_sql_describe_end)
			{
				switch (item)
				{
					case IscCodes.isc_info_truncated:
						...
						info = GetSqlInfo([.. newItems], info.Length);
			...
			while ((item = info[currentPosition++]) != IscCodes.isc_info_sql_describe_end)
			{
				switch (item)
				{
					case IscCodes.isc_info_truncated:
						...
						info = await GetSqlInfoAsync([.. newItems], info.Length, cancellationToken).ConfigureAwait(false);
